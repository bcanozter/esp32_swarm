...
void espnow_data_prepare(uint8_t *buf, const uint8_t *payload,
                         size_t payload_len, bool seq_init) {
  ...
  printf("send seq: %" PRIu32 ", current_seq: %" PRIu32 "\r\n", temp->seq,
         current_seq);
  ESP_LOGW(TAG, "free heap %" PRIu32 ", minimum %" PRIu32 "",
...
static void esp_now_send_timer_cb(TimerHandle_t timer) {
  ...
  if (sent_msgs->max_retry > sent_msgs->retry_times) {
    ...
    if (sent_msgs->sent_msg) {
      ...
      esp_err_t ret =
          esp_mesh_lite_espnow_send(ESPNOW_DATA_TYPE_RESERVE, s_broadcast_mac,
                                    sent_msgs->sent_msg, sent_msgs->msg_len);
      if (ret != ESP_OK) {
...
#endif
{
  ...
  if (status == ESP_NOW_SEND_SUCCESS) {
    ESP_LOGW(TAG, "Send OK to " MACSTR " %s %d", MAC2STR(mac_addr), __func__,
             __LINE__);
  } else {
...
static esp_err_t espnow_recv_cb(const esp_now_recv_info_t *recv_info,
...
static void espnow_task(void *pvParameter) {
  ...
  while (xQueueReceive(espnow_recv_queue, &evt, portMAX_DELAY) == pdTRUE) {
    ...
    case ESPNOW_RECV_CB:
      ...
      memcpy(espnow_payload, buf->payload,
             recv_cb->data_len - ESPNOW_PAYLOAD_HEAD_LEN);
      packet_t *state_data = (packet_t *)espnow_payload;
#if CONFIG_APP_DEBUG
      ESP_LOGI(TAG,
               "Receive broadcast data from: " MACSTR
               ", len: %d, recv_seq: %" PRIu32 ", current_seq: %" PRIu32 "",
               MAC2STR(recv_cb->mac_addr), recv_cb->data_len, recv_seq,
               current_seq);
      ESP_LOGI(TAG, "ID: %u X: %.4f, Y: %.4f, Z: %.4f\n", state_data->id,
               state_data->x, state_data->y, state_data->z);
#endif
      free(recv_cb->data);
      recv_cb->data = NULL;
      break;
...
esp_err_t esp_now_send_broadcast(const uint8_t *payload, size_t payload_len,
                                 bool seq_init) {
  esp_err_t ret = ESP_OK;
  uint8_t *buf = calloc(1, payload_len + ESPNOW_PAYLOAD_HEAD_LEN);
  espnow_data_prepare(buf, payload, payload_len, seq_init);
  app_espnow_create_peer(s_broadcast_mac);
  ret = esp_mesh_lite_espnow_send(ESPNOW_DATA_TYPE_RESERVE, s_broadcast_mac,
                                  buf, payload_len + ESPNOW_PAYLOAD_HEAD_LEN);
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "Send error: %d [%s %d]", ret, __func__, __LINE__);
  }
...
esp_err_t app_espnow_init(void) {
  ESP_LOGE(TAG, "app_espnow_init");
  espnow_recv_queue =
      xQueueCreate(ESPNOW_QUEUE_SIZE, sizeof(esp_mesh_lite_espnow_event_t));
  ...
  esp_mesh_lite_espnow_recv_cb_register(ESPNOW_DATA_TYPE_RESERVE,
                                        espnow_recv_cb);
  ...
  xTaskCreate(espnow_task, "espnow_task", ESPNOW_TASK_STACK_SIZE, NULL,
              ESPNOW_TASK_PRIORITY, &espnow_task_ctrl_handle);
  ...
  TimerHandle_t esp_now_send_timer =
      xTimerCreate("esp_now_send_timer", 100 / portTICK_PERIOD_MS, pdTRUE, NULL,
                   esp_now_send_timer_cb);
